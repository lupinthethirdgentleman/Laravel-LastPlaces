...
<ul class="pagination">
    <li class="{{ ($memberLists->currentPage() == 1) ? ' disabled' : '' }}">
        <a href="{{ ($memberLists->currentPage() == 1) ? 'javascript:void(0)' : str_replace('/?','?',$memberLists->url($memberLists->currentPage()-1)) }}">Previous</a>
    </li>
          ...
          <?php
            ...
            @if ($from < $i && $i < $to)
                <li class="{{ ($memberLists->currentPage() == $i) ? ' active' : '' }}">
                    <a href='{{ str_replace('/?','?',$memberLists->url("$i")) }}'>{{ $i }}</a>
                </li>
            @endif
     @endfor
    <li class="{{ ($memberLists->currentPage() == $memberLists->lastPage()) ? ' disabled' : '' }}">
        <a href="{{ ($memberLists->currentPage() == $memberLists->lastPage()) ? 'javascript:void(0)' : str_replace('/?','?',$memberLists->url($memberLists->currentPage()+1))  }}" >Next</a>
    </li>
</ul>
</div>
@endif
