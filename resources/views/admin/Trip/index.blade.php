	...
	<div id="mws-themer-content" style="<?php echo ($searchVariable) ? 'right: 256px;' : ''; ?>">
		...
		<div class="mws-themer-section">
			<div id="mws-theme-presets-container" class="mws-themer-section">
				<label>{{ trans("messages.$modelName.trip_name") }}</label><br/>
				{{ Form::text('tripname',((isset($searchVariable['tripname'])) ? $searchVariable['tripname'] : ''), ['class' => 'small']) }}
			</div>
		</div>
...
<div  class="mws-panel grid_8 mws-collapsible">
	<div class="mws-panel-header">
		<span>
			...
			<a href='{{URL::to('admin/trip-manager/add-trip')}}'  class="btn btn-success btn-small align">{{ trans("messages.$modelName.add_new") }} </a>
	</div>
	...
	<div class="mws-panel-body testimonial no-padding dataTables_wrapper">
		<table class="mws-table mws-datatable">
			...
			<tbody>
				...
				<tr>
					<td data-th='{{ trans("messages.$modelName.trip_name") }}'>
						{{ $result->tripname }}
					</td> 
					...
					<td  data-th='{{ trans("messages.global.status") }}'>
							@if($result->is_active)
								<a href="{{ URL::to('admin/trip-manager/update-status',array($result->id,0))}}" class="btn btn-success btn-small status_user">{{ trans("messages.global.inactive")}} </a>
							@else
								<a href="{{ URL::to('admin/trip-manager/update-status',array($result->id,1))}}" class="btn btn-warning btn-small  status_user">{{ trans("messages.global.active")}}</a>
							@endif
						<br/>
						<a href='{{URL::to('admin/trip-manager/edit-trip/'.$result->id)}}' class="btn btn-info btn-small mt5">{{ trans("messages.global.edit")}} </a>
						<a href='{{URL::to('admin/trip-manager/delete-trip/'.$result->id)}}' class="btn btn-info btn-small mt5">{{ trans("messages.global.delete")}} </a>
