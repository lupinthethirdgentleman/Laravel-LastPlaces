...
<div id="mws-themer">
	<div id="mws-themer-content" style="<?php echo ($searchVariable) ? 'right: 256px;' : ''; ?>">
		...
		{{ Form::open(['method' => 'get','role' => 'form','url' => 'admin/list-trip-package','class' => 'mws-form']) }}
		{{ Form::hidden('display') }}
		<div class="mws-themer-section">
			<div id="mws-theme-presets-container" class="mws-themer-section">
				<label>{{ trans("Price") }}</label><br/>
				{{ Form::text('price',((isset($searchVariable['price'])) ? $searchVariable['price'] : ''), ['class' => 'small']) }}
			</div>
			<!-- <div id="mws-theme-presets-container" class="mws-themer-section">
		...
		<div class="mws-themer-section">
			...
			<a href="{{URL::to('admin/list-trip-package')}}"  class="btn btn-default btn-small">{{ trans("messages.reset.text") }}</a>
		</div>
		{{ Form::close() }}
	</div>
</div>
...
<div  class="mws-panel grid_8 mws-collapsible">
	<div class="mws-panel-header">
		<span>
			<i class="icon-table"></i> {{ trans("Trip Package") }} </span>
				<a href="{{URL::to('admin/list-trip-package/add-trip-package')}}" class="btn btn-success btn-small align">{{ trans("Add New Trip Package") }} </a>
	</div>
		...
		<table class="mws-table mws-datatable">
			<thead>
				<tr>
					...
					<th width="10%">{{ trans("Supplement") }}</th>
					<th width="10%">
						{{
							link_to_route(
								"TripPackage.index",
			...
			<tbody id="powerwidgets">
				...
				<tr class="items-inner">
					<td data-th='Name'>{{ $record['getTrip']['tripname'] }}</td>
					<td data-th='Heading'>{{ $record->price }}</td>
					<td data-th='Description'>{{ $record->supplement }}</td>
					...
					<td data-th='{{ trans("messages.system_management.action") }}'>
						@if($record->active == 1)
							<a href="{{URL::to('admin/list-trip-package/update-status/'.$record->id)}}" class="btn btn-danger btn-small change_status">Mark inactive</a>
						@else
							<a href="{{URL::to('admin/list-trip-package/update-status/'.$record->id)}}" class="btn btn-success btn-small change_status">Mark active</a>
						@endif
						<!-- <a href="{{URL::to('admin/list-hcp/update-status/'.$record->id)}}" class="btn btn-warning btn-small change_status">Change Status</a> -->
						<a href="{{URL::to('admin/list-trip-package/edit-trip-package/'.$record->id)}}" class="btn btn-info btn-small">{{ trans("messages.system_management.edit") }} </a>
						<!-- <a href="{{URL::to('admin/list-trip-package/view-trip-package/'.$record->id)}}" class="btn btn-info btn-small">{{ trans("messages.system_management.view") }} </a> -->
					</td>
				</tr>
				 @endforeach
				 @else
