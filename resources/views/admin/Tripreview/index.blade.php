	...
	<div id="mws-themer-content" style="<?php echo ($searchVariable) ? 'right: 256px;' : ''; ?>">
		...
		<div class="mws-themer-section">
			<div id="mws-theme-presets-container" class="mws-themer-section">
				<label>{{ trans("messages.$modelName.clientname") }}</label><br/>
				{{ Form::text('clientname',((isset($searchVariable['clientname'])) ? $searchVariable['clientname'] : ''), ['class' => 'small']) }}
			</div>
		</div>
...
<div  class="mws-panel grid_8 mws-collapsible">
	<div class="mws-panel-header">
		<span>
			...
			<a href='{{URL::to('admin/trip-reviews/add-trip')}}'  class="btn btn-success btn-small align">{{ trans("messages.$modelName.add_new") }} </a>
	</div>
			...
			<tbody>
				...
				<tr>
					<td data-th='{{ trans("messages.$modelName.trip_name") }}'>
						{{ $result->trip->tripname }}
					</td> 
					<td data-th='{{ trans("messages.$modelName.clientname") }}'>
							{{ $result->clientname }}
					</td>
					...
					<td  data-th='{{ trans("messages.global.status") }}'>
							@if($result->is_active)
								<a href="{{ URL::to('admin/trip-reviews/update-status',array($result->id,0))}}" class="btn btn-success btn-small status_user">{{ trans("messages.global.inactive")}} </a>
							@else
								<a href="{{ URL::to('admin/trip-reviews/update-status',array($result->id,1))}}" class="btn btn-warning btn-small  status_user">{{ trans("messages.global.active")}}</a>
							@endif
						<br/>
						<a href='{{URL::to('admin/trip-reviews/edit-review/'.$result->id)}}' class="btn btn-info btn-small mt5">{{ trans("messages.global.edit")}} </a>
