 ...
 */
	public function index(){
		...
		// $blocksNews2 = Block::with('description')
		// 				->where('block','home-news-header')
		// 				->first()->toArray();
		...
		$blocksNews = News::with('newsDescription','newsComments','newsAuthor')
						->where('is_active',1)
						->orderBy('created_at')
						->limit(3)
						->get();
		...
		$blocksNewsForHighlight = News::with('newsDescription','newsComments','newsAuthor')
		->where('is_active',1)
		->where('is_highlight',1)
		->orderBy('created_at')
		->limit(3)
		...
		$highlightNewsCount = News::with('newsDescription','newsComments')
								->where('is_active',1)
								->where('is_highlight',1)
								->count();
		...
		return View::make('home.index',compact('blocksCall','blocksDream','blocksBlog','highlightBlogCount','blocksBlog2','blocksBlogForHighlight','highlightCountry','blocksNews','highlightNewsCount','blocksNewsForHighlight'));
	}//end index()
	...
	public function news(){
		...
		$blocksNews = News::with('newsDescription','newsComments','newsAuthor')
						...
						->orderBy('created_at')
						->paginate(2);
		return View::make('news.index',compact('blocksNews'));
	}
	...
	public function setdepartures(){
		...
		/*$tripslist=$DB->join('destination_countrys',function($join){
			...
			$tripslist=	DB::table('trips')
				->select('trips.image','trips.tripname','trips.description','trips.tripdates','trips.tripdays','trips.baseprice','trips.slug','destination_country.name','destination_country.slug as country_slug')
	...
	public function saveComment1(){
		// echo(Input::get('news_id'));
		// die("0");
